...
export function CentersScreen() {
  ...
  return (
    <Screen refreshing={loading} onRefresh={() => void load()}>
      ...
      <AppCard>
        <TextInput placeholder="Search city, region, or center" value={search} onChangeText={setSearch} style={styles.input} />
        ...
        <View style={styles.actions}>
          <AppButton title="Use My Location" variant="outline" onPress={() => void useMyLocation()} />
          <AppButton title="Refresh Centers" loading={loading} onPress={() => void load()} />
        </View>
      </AppCard>

      {message ? <Text style={styles.message}>{message}</Text> : null}
      {!loading && centers.length === 0 ? <AppCard><Text style={styles.muted}>No nearby centers found. Try enabling location or widening your search.</Text></AppCard> : null}
      ...
      {selected ? (
        <AppCard style={styles.detailCard}>
          ...
          <AppButton title="Navigate" variant="outline" onPress={() => void openMap(selected)} />
        </AppCard>
      ) : null}
...
const styles = StyleSheet.create({
  ...
  input: { minHeight: 50, borderWidth: 1, borderColor: colors.border, borderRadius: 14, paddingHorizontal: 14, fontSize: 15, backgroundColor: '#fff' },
  ...
  centerName: { color: colors.ink, fontSize: 18, fontWeight: '900' },
  ...
  nearestCard: { borderColor: '#86efac', backgroundColor: '#f0fdf4' },
